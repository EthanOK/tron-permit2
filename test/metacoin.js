...
contract("MetaCoin", function (accounts) {
  ...
  it("should verify that there are at least two available accounts", async function () {
    ...
    assert.isTrue(
      accounts.length >= 2,
      "At least two accounts are required for testing."
    );
  });

  it("should verify that the contract has been deployed by accounts[0]", async function () {
    const owner = await metaCoinInstance.getOwner();
    assert.equal(
      owner,
  ...
  it("should call a function that depends on a linked library", async function () {
    ...
    const metaCoinConvertedBalance = await metaCoinInstance.getConvertedBalance(
      accounts[0]
    );
    assert.equal(
  ...
  it("should send coins from account 0 to 1", async function () {
    ...
    this.timeout(10000);
    const accountOneStartingBalance = await metaCoinInstance.balanceOf(
      accounts[0]
    );
    const accountTwoStartingBalance = await metaCoinInstance.balanceOf(
      accounts[1]
    );

    const amountToSend = 10;
    await metaCoinInstance.sendCoin(accounts[1], amountToSend, {
      from: accounts[0],
    });

    const accountOneFinalBalance = await metaCoinInstance.balanceOf(
      accounts[0]
    );
    const accountTwoFinalBalance = await metaCoinInstance.balanceOf(
      accounts[1]
    );
  ...
  it("should allow owner to withdraw TRX after unlock time", async function () {
    ...
    const contractBalance = await tronWeb.trx.getBalance(
      metaCoinInstance.address
    );
    ...
    const nonOwnerWithdrawTx = await metaCoinInstance.withdraw(accounts[1], {
      from: accounts[1],
    });
    await waitForTransactionReceipt(nonOwnerWithdrawTx);
    const nonOwnerWithdrawTxInfo = await tronWeb.trx.getTransactionInfo(
      nonOwnerWithdrawTx
    );
    assert.include(
